...
- [x] **Filtros:** O usuário deve ser capaz de filtrar os dados da tabela por **Mês** e por **Categoria** (utilizando um `select-option`).

### 1.2. Gestão de Registros (Lançamentos)

- [x] O usuário deve poder **adicionar** e **remover** registros financeiros.
- [x] Cada registro deve conter obrigatoriamente os seguintes campos:
  - [x] **ID:** Identificador único gerado automaticamente (oculto para o usuário). Sugestão: biblioteca **uuid** ou **crypto.randomUUID()**.
  - [x] **Descrição:** Texto explicativo do lançamento (ex: "Conta de Luz", "Salário").
  - [x] **Tipo:** Entrada, Saída ou Investimento.
  - [x] **Categoria:** Categoria customizável atrelada ao tipo.
  - [x] **Data:** Data da ocorrência. _(Regra: O sistema não deve permitir o cadastro de datas futuras para despesas e receitas)._
  - [x] **Valor:** Valor monetário do registro.
...
- [x] **Componentização:** A aplicação deve ser dividida em múltiplos componentes de forma lógica e reutilizável.
  - [x] _Exemplos sugeridos:_ Um componente para o Cabeçalho/Menu, um componente para os Cards de Resumo, um componente para o Formulário, um componente para a Tabela.
- [x] **Comunicação entre Componentes (Input/Output):** É obrigatório demonstrar o fluxo de dados entre componentes Pai e Filho utilizando os decorators @Input() e @Output() (com EventEmitter).
  - [x] Exemplo: O componente pai `<app-dashboard>` passa os dados filtrados via @Input() para o componente filho `<app-tabela-registros>`, e a tabela emite um evento de exclusão via @Output() para o pai.
- [x] **Persistência de Dados:** Salvar os registros e configurações no localStorage. Utilizar um Service (com @Injectable()) para centralizar a lógica de leitura e gravação no storage. A carga inicial pode ser feita através de um mock caso o storage esteja vazio.
- [x] **Manipulação de Datas:** Utilizar a biblioteca **moment.js** (ou nativa) para formatar e calcular intervalos.
- [x] **Estilização e Responsividade:** O layout deve ser responsivo. O uso de bibliotecas compatíveis com Angular (como Angular Material, PrimeNG, TailwindCSS ou Bootstrap) é recomendado.
